...
-- workspace_invites_pending_idx is UNIQUE (workspace_id, email) WHERE
-- accepted_at IS NULL, and the settings page depends on it: it catches 23505
-- to say "already has an invitation waiting". Nulls compare as distinct by
-- default, so the moment workspace_id can be null that guard silently stops
-- applying to creation-only invites and they stack up unnoticed.
drop index public.workspace_invites_pending_idx;
create unique index workspace_invites_pending_idx
  on public.workspace_invites (workspace_id, email) nulls not distinct
