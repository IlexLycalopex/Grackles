...
function describes(candidate: Partial_, want: Want): boolean {
  ...
  // Both credits go in as written. `creditMatches` folds them itself, and has
  // to see the punctuation to do it: a comma is how "Le Guin, Ursula K." says
  // it is back to front, and how "Italo Calvino, William Weaver" says it holds
  // a translator. `normalise` deletes both, and the field stops being readable.
  return creditMatches(candidate.author ?? '', want.author);
}
