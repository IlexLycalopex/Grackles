...
async function fromItunes(album: string, artist: string): Promise<string> {
  ...
  for (const result of [...combined, ...byTitle]) {
    ...
    //
    // The two credits go in as written: `creditMatches` needs the ampersand to
    // see "DJ Shadow & Cut Chemist" as two acts rather than one long name, and
    // normalising first would have already thrown it away.
    if (!creditMatches(result.artistName ?? '', artist) || !looselyEqual(gotAlbum, wantAlbum))
      continue;
